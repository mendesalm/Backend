...
export const createAviso = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao criar aviso.", errorDetails: error.message });
  }
};
...
export const getAllAvisos = async (req, res) => {
  try {
    ...
    const avisos = await db.Aviso.findAll({
      ...
      include: [
        { model: db.LodgeMember, as: "autor", attributes: ["NomeCompleto"] },
      ],
      order: [
        ["fixado", "DESC"],
        ["createdAt", "DESC"],
    ...
    res
      .status(500)
      .json({ message: "Erro ao listar avisos.", errorDetails: error.message });
  }
};
...
export const getAvisoById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao buscar aviso.", errorDetails: error.message });
  }
};
...
export const updateAviso = async (req, res) => {
  try {
    const [updated] = await db.Aviso.update(req.body, {
...
export const deleteAviso = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao deletar aviso.", errorDetails: error.message });
  }
};

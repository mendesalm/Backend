...
export const getAllPatrimonios = async (req, res) => {
  ...
  try {
    ...
    const itens = await db.Patrimonio.findAll({
      where: whereClause,
      include: [{ model: db.LodgeMember, as: 'registradoPor', attributes: ['NomeCompleto'] }],
      order: [['nome', 'ASC']],
    });
...
export const updatePatrimonio = async (req, res) => {
  try {
    // CORREÇÃO: Aceder ao modelo através de `db.Patrimonio`
    const [updated] = await db.Patrimonio.update(req.body, { where: { id: req.params.id } });
...
export const deletePatrimonio = async (req, res) => {
  try {
    // CORREÇÃO: Aceder ao modelo através de `db.Patrimonio`
    const deleted = await db.Patrimonio.destroy({ where: { id: req.params.id } });

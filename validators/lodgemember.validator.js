...
export const updateMyProfileRules = () => {
  return [
    // Certifique-se que este array está sendo retornado
    body("NomeCompleto")
      .optional()
      .trim()
      .notEmpty()
    ...
    body("Email")
      ...
      .custom(async (value, { req }) => {
        ...
        if (!LodgeMember)
          throw new Error(
            "Modelo LodgeMember não inicializado para validação de Email."
          );
    ...
    body("CIM")
      ...
      .trim()
      .notEmpty()
      .withMessage("CIM não pode ser vazio se fornecido.")
      .custom(async (value, { req }) => {
        if (!value) return true;
        const { LodgeMember } = db;
        if (!LodgeMember)
          throw new Error(
            "Modelo LodgeMember não inicializado para validação de CIM."
          );
    ...
    body("Identidade")
      ...
      .trim()
      .notEmpty()
      .withMessage("Identidade não pode ser vazia se fornecida."),
    body("DataNascimento")
      .optional({ checkFalsy: true })
      .isISO8601()
    ...
    body("DataCasamento")
      ...
      .isISO8601()
      .toDate()
      .withMessage("Data de casamento inválida."),
    body("Endereco_CEP")
      .optional({ checkFalsy: true })
      .trim()
      .isLength({ min: 8, max: 9 })
      .withMessage("CEP deve ter 8 ou 9 caracteres."),
...
export const createLodgeMemberRules = () => {
  return [
    ...
    body("CPF")
      .trim()
      .optional({ checkFalsy: true }) // <-- MUDANÇA 1: Torna o campo opcional (aceita nulo, undefined ou string vazia)
      .custom((value) => {
        // MUDANÇA 2: A validação customizada só roda se um valor existir
        if (value && !isValidCPF(value)) {
          throw new Error("O CPF fornecido é inválido.");
        }
        return true; // Retorna true se o campo estiver vazio ou se o CPF for válido
      }),
    body("SenhaHash")
      .notEmpty()
      .withMessage("Senha é obrigatória.")
      .isLength({ min: 8, max: 100 })
      .withMessage("A senha deve ter entre 8 e 100 caracteres.")
      .matches(/\d/)
    ...
    body("credencialAcesso")
      .notEmpty()
      .withMessage("Credencial de acesso é obrigatória.")
      .isIn(["Webmaster", "Diretoria", "Membro"])
      .withMessage("Credencial de acesso inválida."),
    body("Graduacao")
      .optional({ checkFalsy: true })
      .isIn(["Aprendiz", "Companheiro", "Mestre", "Mestre Instalado"])
      .withMessage("Graduação inválida."),
    body("DataNascimento")
      .optional({ checkFalsy: true })
      .isISO8601()
      .toDate()
...
export const updateLodgeMemberByAdminRules = () => {
  return [
    param("id")
      .isInt({ gt: 0 })
      .withMessage(
        "ID do maçom no parâmetro da URL deve ser um inteiro positivo."
      ),
    body("NomeCompleto")
      .optional()
      .trim()
    ...
    body("CPF")
      ...
      .trim()
      .notEmpty()
      .withMessage("CPF não pode ser vazio se fornecido.")
      .custom(async (value, { req }) => {
        if (!value) return true;
...
export const lodgeMemberIdParamRule = () => {
  return [
    param("id")
      .isInt({ gt: 0 })
      .withMessage("ID do maçom na URL deve ser um inteiro positivo."),
  ];
};
